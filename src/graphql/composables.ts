...
import { SEARCH_MUSHROOMS, GET_SHROOM_BY_ID, GET_RANDOM_FUNGI, SEARCH_MUSHROOM_NAMES, GET_LOOK_ALIKE_FUNGI } from "@/graphql/queries"
...
export function useMushroomLookAlikes(lookAlikeIds: Ref<number[]>) {
  const variables = ref<{ ids: number[] }>({ ids: [] })
  const enabled = computed(() => variables.value.ids.length > 0)
  watch(
    lookAlikeIds,
    (ids) => {
      variables.value = { ids: Array.isArray(ids) ? ids.slice() : [] }
    },
    { immediate: true }
  )
  const { result, loading, error, refetch } = useQuery(GET_LOOK_ALIKE_FUNGI, variables, { enabled })
  const lookAlikes = ref<any[]>([])
  watch(
    () => result.value,
    (val) => {
      if (!val?.attributesCollection?.edges) {
        lookAlikes.value = []
        return
      }
      lookAlikes.value = val.attributesCollection.edges
        .map((e: any) => flattenFungi(e.node?.fungi))
        .filter(Boolean)
    },
    { immediate: true }
  )
  return { lookAlikes, loading, error, refetch }
}
